  ...
  return (
    <div className="flex flex-col h-screen w-screen p-4 space-y-4 overflow-hidden bg-white text-slate-900 relative">
      <header className="flex justify-between items-center shrink-0 h-20">
        ...
        <div className="flex items-center gap-6">
          <button onClick={() => setCurrentView(v => v === 0 ? 1 : 0)} className="px-6 py-3 bg-[#2fabab] text-white rounded-2xl font-bold uppercase text-xs tracking-widest shadow-lg transition-transform active:scale-95">Trocar Visão</button>
      ...
      <div className="flex-1 min-h-0 relative">
        <div className={`absolute inset-0 transition-all duration-700 flex flex-col space-y-6 ${currentView === 0 ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
        ...
        <div className={`absolute inset-0 transition-all duration-700 grid grid-cols-4 gap-6 ${currentView === 1 ? 'opacity-100 scale-100' : 'opacity-0 scale-105 pointer-events-none'}`}>
